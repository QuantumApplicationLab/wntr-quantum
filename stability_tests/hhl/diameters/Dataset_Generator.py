...
benchmark = os.getcwd() + '/Pipe_Diameters_Scenarios_'
...
def quantum_algo(wn):
    ...
    estimator = Estimator()
    sampler = Sampler()
    linear_solver = HHL_SOLVER(estimator=estimator, sampler=sampler)
    sim = wntr_quantum.sim.QuantumEpanetSimulator(wn, linear_solver=linear_solver)
    return sim.run_sim(linear_solver=linear_solver)


def runPipeDiameterScenarios(scNum):
    """XXX"""
    ...
    while itsok is not True:
        ...
        try:
            # pipe diameter must vary by +- 10%
            uncertainty_Diameter = 0.10
            ...
            # set uncertainty in Diameters
            tempDiameters = wn.query_link_attribute('diameter')
            tempDiameters = np.array([tempDiameters.iloc[i] for i, _ in enumerate(tempDiameters)])
            ...
            sorted_diameters = ql + np.random.rand(dem_diameter) * (qu - ql)

            #print(tempDiameters, sorted_diameters, uncertainty_Diameter)

            for n, diameter in enumerate(sorted_diameters):
                #print(n, wn.get_link(wn.link_name_list[n]).diameter)
                wn.get_link(wn.link_name_list[n]).diameter = diameter
                print(n, wn.get_link(wn.link_name_list[n]).diameter*1000)
